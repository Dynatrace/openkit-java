...
public class OpenKitImpl implements OpenKit {
    ...
    protected OpenKitImpl(Logger logger, Configuration config, HTTPClientProvider httpClientProvider, TimingProvider timingProvider, ThreadIDProvider threadIDProvider) {
        if (logger.isInfoEnabled()) {
            ...
            logger.info("OpenKitImpl() - " + config.getOpenKitType() + " OpenKit " + OpenKitConstants.DEFAULT_APPLICATION_VERSION
                    + " instantiated");
        }
        if (logger.isDebugEnabled()) {
            logger.debug(
                "OpenKitImpl() - applicationName=" + config.getApplicationName() + ", applicationID=" + config.getApplicationID()
    ...
    public Session createSession(String clientIPAddress) {
        if (logger.isDebugEnabled()) {
            logger.debug("OpenKitImpl createSession(" + clientIPAddress + ")");
        }
        if (isShutdown.get()) {
            return NULL_SESSION;
        }
    ...
    public void shutdown() {
        if (logger.isDebugEnabled()) {
            logger.debug("OpenKitIml shutdown() - shutdown requested");
        }
        isShutdown.set(true);
